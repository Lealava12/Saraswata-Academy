...
class ExamController extends Controller
{
    ...
    {
        $exams = Exam::with(['classInfo.board', 'subject'])->latest()->get();
        return view('admin.exams.index', compact('exams'));
    }

    public function create()
    {
        $classes = Classes::with('board')->where('is_active', 1)->get();
    ...
    {
        $exam->load(['classInfo.board', 'subject']);
     ...
     */    public function exportMarksPdf(Exam $exam)    {
        ...
        $filename = 'marks_' . ($exam->classInfo->name ?? 'class') . ($exam->classInfo->board->name ? '_' . $exam->classInfo->board->name : '') . '_' . $exam->subject->name . '_' .
            now()->format('Y-m-d') . '.pdf';

        return $pdf->download($filename);    }    public function exportCsv()    {
        $exams = Exam::with(['classInfo.board', 'subject'])->latest()->get();
        ...
        $callback = function () use ($exams) {
            ...
            foreach ($exams as $i => $exam) {
                fputcsv($file, [
                    $i + 1,
                    ($exam->classInfo->name ?? '-') . ($exam->classInfo->board->name ? ' (' . $exam->classInfo->board->name . ')' : ''),
                    $exam->subject->name ?? '-',
                    $exam->exam_date,
                    $exam->full_marks,
